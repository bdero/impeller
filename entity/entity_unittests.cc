...
TEST_F(EntityTest, BadCubicCurveTest) {
...
TEST_F(EntityTest, BadCubicCurveAndOverlapTest) {
  EntityPlaygroundCallback callback = [&](ContentContext& context,
    ...
    {
      static bool on;
      ImGui::Checkbox("Path0", &on);
      if (on) {
        path.MoveTo({359.934, 96.6335})
            ...
            .CubicCurveTo({312.528, 102.001}, {310.687, 102.366},
                          {308.846, 102.748})
            .CubicCurveTo({307.85, 102.955}, {306.855, 103.182},
                          {305.859, 103.4})
            ...
            .CubicCurveTo({301.388, 104.409}, {300.366, 104.67},
                          {299.344, 104.921})
            .CubicCurveTo({298.618, 105.1}, {297.89, 105.269},
                          {297.165, 105.455})
            ...
            .CubicCurveTo({251.844, 119.849}, {250.056, 120.474},
                          {248.189, 121.131})
            .CubicCurveTo({248, 121.197}, {247.812, 121.264},
                          {247.621, 121.331})
            ...
            .CubicCurveTo({240.659, 123.801}, {240.357, 123.909},
                          {240.052, 124.018})
            .CubicCurveTo({239.12, 124.351}, {238.18, 124.687},
                          {237.22, 125.032})
    ...
    {
      static bool on;
      ImGui::Checkbox("Path1", &on);
      if (on) {
        path.MoveTo({340.082, 121.849})
            ...
            .CubicCurveTo({338.532, 123.959}, {338.554, 123.79},
                          {338.58, 123.626})
            .CubicCurveTo({338.58, 123.625}, {338.58, 123.625},
                          {338.58, 123.625})
    ...
    {
      static bool on;
      ImGui::Checkbox("Path2", &on);
      if (on) {
        path.MoveTo({340.678, 113.245})
            ...
            .CubicCurveTo({331.135, 111.755}, {333.219, 112.946},
                          {334.526, 113.833})
            .CubicCurveTo({334.54, 113.816}, {334.554, 113.8},
                          {334.569, 113.784})
    ...
    {
      static bool on;
      ImGui::Checkbox("Path3", &on);
      if (on) {
        path.MoveTo({346.357, 106.771})
    ...
    {
      static bool on;
      ImGui::Checkbox("Path4", &on);
      if (on) {
        path.MoveTo({347.56, 106.771})
...
TEST_F(EntityTest, BadCubicCurveAndOverlapTest2) {
  EntityPlaygroundCallback callback = [&](ContentContext& context,
                                          RenderPass& pass) -> bool {
    // Compare with https://fiddle.skia.org/c/a75d5da581da6b9683d846f7ad6911dc
    PathBuilder path = PathBuilder{};
    path.MoveTo({10, 10})
        .LineTo({10, 210})
        .LineTo({210, 210})
...
TEST_F(EntityTest, SimpleStrokeWithMove) {
  // Compare with https://fiddle.skia.org/c/a75d5da581da6b9683d846f7ad6911dc
  Path path =
      PathBuilder{}
          .MoveTo({10, 10})
